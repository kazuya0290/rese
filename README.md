...
・<strong>管理者画面(新店舗追加の為のCSVファイル記入例)</strong>：新店舗追加ファイル.csv(src > public)の「店舗名、地域、ジャンル、店舗概要、画像URL」カラムの下(2行目)に値をそれぞれ入力することで新店舗を追加することが可能です。<strong>※1行目のカラム名を読み取ってデータ登録を行う為、カラム名を削除したり、変更したりしないでください。</strong>店舗名は50文字以内、店舗概要は400文字以内、画像URLは後述のURLを1つ選択して入力する必要があります。地域とジャンルはそれぞれ「東京都」「大阪府」「福岡県」、「寿司」「焼肉」「居酒屋」「イタリアン」「ラーメン」と記入するのでは無く、<strong>それぞれ半角数字で、「1,2,3」、「1,2,3,4,5」のうち1つを記載する必要があります。</strong>割り当ては以下の通りです。
```bash
地域： 1 →　東京　2　→　大阪 3　→　福岡
