...
vscode内でも記述可能で、画像の通り、半角の「,」で区切って記載し、インポートボタンをクリックすることで新店舗情報を追加することが可能です。<strong>※1行目のカラム名を読み取ってデータ登録を行うため、カラム名を削除したり、変更したりしないでください。店舗名は50文字以内、店舗概要は400文字以内、画像URLは後述のURLまたはjpeg,pngファイルを1つ選んで入力してください。地域とジャンルはそれぞれ半角数字で入力してください。地域は「1, 2, 3」、ジャンルは「1, 2, 3, 4, 5」のいずれか1つを選んでください。</strong><br>
・地域の割り当て<br>
```bash
 東京: 1
 大阪: 2 
