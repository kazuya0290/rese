...
Rese飲食店予約サービスアプリは、利用者(ユーザー)が飲食店をエリアやジャンル選択、キーワード検索で簡単に検索し、オンラインで予約できるサービスです。<br> 検索機能は、プルダウンボックスからエリアやジャンルを選択して絞り込むフィルター機能と、キーワード検索機能、店舗のランダム表示、評価の高いまたは低い順に店舗を表示するソート機能の3つを提供し、ユーザーが希望の飲食店を迅速にかつ楽しく見つけることができます。<br> その他にも、お気に入り店舗の追加、予約管理、QRコード発行、オンライン決済(Stripe決済)、リマインダーメール機能、口コミ投稿・編集・削除機能など、多彩な機能を提供し、ユーザーの利便性を高めています。<br> また、管理者は店舗代表者の追加・利用者(ユーザー)への一斉メール送信、利用者(ユーザー)の記載した全店舗の口コミの確認・削除、CSVファイルをインポートすることによる店舗情報の追加、店舗代表者は店舗情報の更新や新規追加が可能で、店舗運営をサポートします。<br>
...
<strong>管理者画面(CSVファイルインポート方法)</strong>
![管理者画面(CSVファイルインポート方法)](https://github.com/user-attachments/assets/ac17b633-4152-4193-b5e8-feb5acdae4be)

<strong>管理者画面(CSVファイルインポート時のバリデーション)</strong>
![管理者画面(インポート時のバリデーション)](https://github.com/user-attachments/assets/c3605936-cf95-4442-b4c9-566e6f010568)
...
・<strong>ホーム画面(店舗一覧)</strong>：各店舗を表示する画面です。画面右上の店舗のランダムまたは評価の高いまたは低い順に表示するソート機能、エリア・ジャンル選択、キーワード検索によるフィルター機能は非ログイン時でも利用可能です。各店舗の「詳しくみる」ボタンをクリックすると、店舗詳細情報を閲覧することが出来ます。<br>
...
・<strong>店舗詳細画面</strong>：店舗の画像、エリア・ジャンルのハッシュタグ、店舗の詳細な説明が記載されてあります。「<」をクリックすると一覧に戻り、「画像を保存する」ボタンをクリックするとストレージに画像を保存することが可能で、非ログイン時でも画像の保存が可能です。右側の予約フォームは非ログイン時には利用することが出来ませんが、ログイン時には上から順に日付、時間、人数をそれぞれ選択することで予約を行うことが可能です。また、予約方法は通常予約とStripe決済での予約を選ぶことが出来ます。「口コミを投稿する」リンクも予約フォームと同じく非ログイン時には表示されませんが、ログインすることで表示され、ここから新たな口コミを記載することが可能です。後述の予約リマインダーメールのリンクからも該当する店舗の口コミを投稿することが可能です。<br>
...
・<strong>予約リマインダーメール</strong> : 予約当日の朝にリマインダーメールをmailhog(localhost:8025)に送信できます。Kernel.php(src > app > Console)で、朝6時にメールが届くよう設定していますが、dailyAt('06:00')部分を変更すれば配信時間を調整可能です(例: 朝8時に送信する場合は8:00に設定)。メールは自動送信されないため、ターミナルで以下のコマンドを実行してください：<br>
```bash
php artisan schedule:run
```
・<strong>予約リマインダーメールの内容</strong>：予約した店舗名、日時、人数が記載されております。 また、リンクになっている「レビューする」をクリックするとその店舗の口コミ投稿画面が表示されます。<br>

・<strong>口コミ投稿画面</strong>：各店舗の５段階評価、コメントを記入することが可能です。５段階評価はカーソルを星に合わせてクリックすることで評価することが可能となっております。また、画像の投稿も可能(必須)となっており、「クリックして画像を追加またはドラッグアンドドロップ」エリアをクリックするかエリア内に画像を直接ドラッグアンドドロップすることで画像を挿入することが可能となります。口コミは1人のユーザーが1つの店舗で2件以上口コミを記載することが出来ない仕組みとなっております。<br>

・<strong>口コミ投稿サンクス画面</strong>：無事に口コミの投稿が完了すると表示されます。「マイページへ戻る」をクリックするとマイページ画面が表示されます。<br>

・<strong>口コミ内容確認・編集・削除画面</strong>：各店舗の詳細画面に口コミ投稿画面にて評価・コメントした情報がこちらで反映されます。「(〇件の口コミ)」をクリックすることで投稿された日付、ユーザー名に加え、記載されたコメントがモーダルウィンドウで表示されます。画面上の×ボタンをクリックすることで閉じることが出来ますが、画面のどの部分をクリックしても画面を閉じることが可能です。また、編集ボタンをクリックすると口コミの編集画面が表示され、その画面から口コミの編集を行うことが出来、削除ボタンをクリックすると口コミを記載した店舗の口コミを削除することが可能です。口コミの編集・削除は口コミを記入したユーザーだけが行える仕組みとなっており、他のユーザーがログイン後にその口コミを確認しても編集・削除ボタンは表示されません。<br>

・<strong>口コミ編集画面</strong>：「(〇件の口コミ)」をクリックすることで表示されるモーダルウィンドウ内の編集ボタンをクリックすることで、前回の口コミの入力値を保持した状態で口コミ編集画面が表示され、口コミを編集することが可能です。<br>
...
・<strong>管理者画面(利用者(ユーザー)の全店舗の口コミ確認・削除とCSVファイルインポート機能)</strong>：利用者(ユーザー)の全店舗の口コミ確認・削除とCSVファイルインポート機能です。「全店舗の口コミを表示」ボタンをクリックするとモーダルウィンドウが表示され、全店舗の口コミの確認・削除を行うことが可能です。また、下部のインポートボタンは、新店舗追加ファイル.csv(src > public)に値を入力してインポートボタンをクリックすることで新店舗情報を追加することが可能です。<br>

・<strong>管理者画面(利用者(ユーザー)の全店舗の口コミ確認・削除)</strong>：口コミを投稿した利用者(ユーザー)名、年月日、全店舗名、口コミの内容、投稿した画像を確認することが可能です。削除ボタンをクリックすることで自由に口コミを削除することが可能です。<br>

・<strong>管理者画面(CSVファイルインポート方法)</strong>：新店舗追加ファイル.csv(src > public)の「店舗名、地域、ジャンル、店舗概要、画像URL」カラムの下(2行目)に値をそれぞれ入力することで新店舗を追加することが可能です。店舗名は50文字以内、店舗概要は400文字以内、画像URLは後述のURLを選択して入力する必要があります。地域とジャンルはそれぞれ「東京都」「大阪府」「福岡県」、「寿司」「焼肉」「居酒屋」「イタリアン」「ラーメン」と記入するのでは無く、それぞれ半角数字で、「1,2,3」、「1,2,3,4,5」と記載する必要があります。割り当ては以下の通りです。
```bash
地域： 1 →　東京　2　→　大阪 3　→　福岡
ジャンル: 1 → 寿司 2 → 焼肉 3 → 居酒屋 4 → イタリアン 5 → ラーメン　
画像URL　↓
...
```
これらを全て記入し、インポートボタンをクリックすることで、新店舗情報のインポートが完了致します。<br>
