...
・会員登録画面：上から順に名前、メールアドレス、パスワード(確認用にも同じパスワードを入力する必要あり)を入力し、メール認証を行うことによって会員登録を行うことが可能です。<br>

・メール認証画面 : この画面が出た後、mailhog(localhost:8025)にメールが届くので、Verify Email Addressをクリックして認証を行うことで会員登録が完了し、ログインすることが出来ます。<br>
